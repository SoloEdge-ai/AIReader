...
聊天面板用书籍／会话双重过滤的 `turn` 事件更新回答，不再每秒轮询完整会话。事件在 renderer 中按到达顺序短暂排队；若 HTTP 快照读取期间收到新事件，以新事件覆盖该快照中的旧回答。WebSocket 断线后重新建立会话并连接；重新联网或窗口恢复可见时也读取会话快照，补齐断线期间未收到的变化。Core 不保证事件重放，HTTP 快照仍是恢复事实来源；账号状态目前仍由独立状态层轮询。
