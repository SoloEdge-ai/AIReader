...
workspace v5 的主题组包含 `id/title/color/x/y/width/height/memberIds/collapsed`。成员只能是本书工作区中的卡片或完全位于白板的绘图对象，一个对象最多属于一个组，组不嵌套。组折叠只改变呈现；关系仍保存真实成员端点，组本身不是关系端点。删除卡片或对象时，同一命令清理所有组成员引用；删除组不删除成员。Note 生命周期删除和恢复位置时也在同一事务保存并恢复其组归属。
