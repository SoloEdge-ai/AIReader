...
- Windows CI：`36120839503` 在 `2482aaf` 完整通过，覆盖修正后的聊天事件补读、回答笔记、PDF 区域缩放、桌面关闭和安装更新；`b74b9bf` 的 `36121994924` 因上述笔记卡片同步竞态失败，`507500c` 的 `36124407544` 因工具盘测试页一次未挂载失败。`36119339601` 暴露过 Core 完成而聊天 UI 停在等待的终态事件缺失，已通过进行中轮次补读和故障注入 UI 测试修复。
