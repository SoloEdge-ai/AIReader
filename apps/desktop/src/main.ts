...
app.whenReady().then(() => {
  ...
  core = utilityProcess.fork(join(__dirname,'../core/main.cjs'), [], {env:{...process.env,AIREADER_PORT:'0',AIREADER_WEB:join(__dirname,'../web'),AIREADER_DATA:app.getPath('userData')},serviceName:'AIReader Core'});
