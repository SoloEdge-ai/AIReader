...
export function validateWorkspaceGroups(workspace: GroupWorkspace) {
  const cardIds = new Set(workspace.cards.filter((card) => card.placed !== false).map((card) => card.id));
  const objects = new Map(workspace.objects.map((object) => [object.id, object]));
  const contentIds = new Set([...workspace.cards.map((card) => card.id), ...objects.keys()]);
