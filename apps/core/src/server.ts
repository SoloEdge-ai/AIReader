...
export function createCore(directory:string,webRoot:string){
  ...
  try{
   ...
   if(parts[0]==='api'){
    ...
    if(parts[1]==='ai'){
     ...
     if(req.method==='POST'&&parts[2]==='path'){const value=z.object({path:z.string().max(1000)}).parse(await jsonBody(req));bookTools.close();indexer.close();chat.close();codex.disconnect();library.store.put('setting','codex','',value);codex=new CodexAdapter(join(directory,'control'),value.path||undefined);chat=new ChatService(library,codex);indexer=new IndexService(library,codex);bookTools=new BookTools(library,codex);send(res,{ok:true});return;}
    }
    ...
    if(parts[1]==='books'&&parts[2]){
     ...
     if(parts[3]==='tools'){
      if(req.method==='GET'){send(res,{...bookTools.status(id),files:await bookTools.files(id)});return;}
      if(req.method==='POST'&&parts[4]==='verify'){send(res,await bookTools.verify(id));return;}
      if(req.method==='POST'&&parts[4]==='run'){const value=z.object({turnId:z.string(),script:z.string().min(1).max(12000)}).parse(await jsonBody(req));send(res,await bookTools.run(id,value.turnId,value.script));return;}
      if(req.method==='POST'&&parts[4]==='stop'){const value=z.object({turnId:z.string()}).parse(await jsonBody(req));const turn=library.store.get<import('../../../packages/protocol/src').ChatTurn>('turn',value.turnId);if(turn?.bookId!==id)throw new Error('会话与书籍不匹配');await bookTools.cancel(value.turnId);send(res,{ok:true});return;}
      if(req.method==='POST'&&parts[4]==='draft'){const value=z.object({task:z.string().min(1).max(2000)}).parse(await jsonBody(req));const result=await codex.answer('生成一段完成用户任务的 PowerShell 脚本，只返回代码，不执行。工作区仅有 evidence.txt，输出文件必须写在当前目录。禁止联网、读取工作区以外文件和安装依赖。用户任务：'+value.task);send(res,{script:result.text.replace(/^```(?:powershell|ps1)?\s*/,'').replace(/\s*```$/,'')});return;}
     }
     if(parts[3]==='generated'&&req.method==='GET'){const file=await bookTools.file(id,url.searchParams.get('name')??'');res.setHeader('Content-Type','application/octet-stream');res.setHeader('Content-Disposition',"attachment; filename*=UTF-8''"+encodeURIComponent(url.searchParams.get('name')??'file'));createReadStream(file).pipe(res);return;}
     if(parts[3]==='index'){
 ...
 return{server,library,emit,close(){bookTools.close();indexer.close();chat.close();codex.disconnect();for(const client of sockets.clients)client.terminate();sockets.close();server.close();library.close();}};
}
