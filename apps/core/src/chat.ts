...
export class ChatService {
 ...
 start(reading:ReadingSnapshot,question:string,sessionId:string,model?:string){
  ...
  void this.codex.answer(renderPrompt(context,question),{signal:control.signal,model,onText:text=>{if(turn.status==='running'){turn.answer=text;this.save(turn);}}}).then(result=>{if(control.signal.aborted||this.closed)return;Object.assign(turn,validateCitations(result.text,context),{status:'complete',usage:result.usage});const key=turn.bookId+':'+sessionId;const previous=this.library.store.get<{goal?:string}>('memory',key);const recentQuestions=[...this.list(turn.bookId,sessionId).filter(t=>t.status==='complete').slice(-5).map(t=>t.question),question];this.library.store.put('memory',key,turn.bookId,{goal:previous?.goal??'',text:`学习目标：${previous?.goal??'尚未指定'}\n最近关注的问题（不代表已验证事实）：\n${recentQuestions.map(q=>q.slice(0,180)).join('\n')}`.slice(0,1600)});}).catch(error=>{turn.status=control.signal.aborted?'cancelled':'error';turn.error=error.message;}).finally(()=>{this.running.delete(turn.id);this.save(turn);});return turn;
 }
