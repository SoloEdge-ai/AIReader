...
function Page({
  ...
  const [size, setSize] = useState({ width: 660, height: 930 });
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        setVisible(entry.isIntersecting);
      },
      { rootMargin: "250px", threshold: [0, 0.25, 0.6] },
    );
  ...
  useEffect(() => {
    ...
    void pdf.getPage(page).then((p) => {
      if (active) {
        const view = p.getViewport({ scale: zoom });
  ...
  useEffect(() => {
    ...
    if (visible)
      void (async () => {
        ...
        if (cancelled) return;
        text.current!.replaceChildren();
        text.current!.style.setProperty("--scale-factor", String(zoom));
...
export function PdfReader({
  ...
  useEffect(() => {
    ...
    const report = () => {
      ...
      const current = pages.find((p) => p.getBoundingClientRect().bottom > top);
      if (current) onPage(Number(current.dataset.page));
    };
    const onScroll = () => {
      if (!pending) {
        pending = true;
