...
import {
  clampChatWindow, defaultChatWindow, moveChatWindow, resizeChatWindow,
...
export function FloatingChatWindow({ rect, bounds, onCommit, onClose, children }: {
  ...
  useEffect(() => {
    if (!gesture.current) show(clampChatWindow(rect ?? defaultChatWindow(bounds), bounds));
  ...
  return <aside className="floating-chat" aria-label="AI 问答浮窗" style={{
    ...
    {edges.map((edge) => <div key={edge} className="floating-chat-resize" data-edge={edge}
      role="separator" tabIndex={0} aria-label={`调整问答浮窗${edgeNames[edge]}边缘`}
      title="拖动调整大小 · 方向键微调"
