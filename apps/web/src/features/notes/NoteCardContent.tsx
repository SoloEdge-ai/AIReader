...
export function NoteCardContent({ note, state, editing }: { note?: Note; state: BookNotes; editing: boolean }) {
  ...
  return <div className="workspace-note-content">
    ...
    {editing ? <NoteEditor key={note.id} note={note} state={state} /> : <>
      <strong>{note.title}</strong><p className="workspace-note-preview">{text(note.document) || "点击编辑笔记…"}</p>
    </>}
  </div>;
}
